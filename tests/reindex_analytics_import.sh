...
curl -s -u $TOMCAT_HOST_USERNAME:$TOMCAT_HOST_PASSWORD "http://$TOMCAT_HOST/gxa/admin/experiments/$EXP_ACC/ANALYTICS_IMPORT" 
if [ $? -ne 0 ]; then
    echo "ERROR: analytics indexing for $EXP_ACC on $TOMCAT_HOST"
    exit 1
fi
